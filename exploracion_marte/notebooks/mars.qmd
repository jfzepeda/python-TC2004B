...
def setMaxDeltaH(nmdh):
  global max_delta_h
  max_delta_h = nmdh
setMaxDeltaH(MAX_HEIGHT_DIFF)
...
def setStartEnd(start_x, start_y, end_x, end_y):
...
def calcHeur(r, c, tx=None, ty=None):
  if tx is None:
    tx = END_X
  if ty is None:
    ty = END_Y
...
def path_stats(route: List[Node]):
  if len(route) < 2:
    return {'steps': 0, 'turns': 0, 'diag_steps': 0, 'axis_steps': 0, 'max_delta_h': 0.0}
...
def bfs_search():
  ...
  while q:
    ...
    for nn in getNeigbors(curr):
      ...
      child = Node(nn.row, nn.col, nn.height, nn.heur, nn.cost, curr)
...
```{python}
def showResults(res_bfs, res_dij, res_ast, plot):
  plot_arry = []
  if res_bfs:
    plot_arry.append(('BFS', res_bfs))
    print('BFS:')
    ...
    if res_bfs.found:
      ...
      print(f'  Pasos: {stats["steps"]}, giros: {stats["turns"]}, diagonales: {stats["diag_steps"]}, rectos: {stats["axis_steps"]}, Δaltura máx: {stats["max_delta_h"]:.4f} m')

  if res_dij:
    plot_arry.append(('Dijkstra', res_dij))
    print('\nDijkstra:')
  ...
  if plot:
    plot_three_results(plot_arry)

def main(start_x=2850, start_y=6400, end_x=3150, end_y=6800, max_dh=0.25):
  setStartEnd(start_x, start_y, end_x, end_y)
  setMaxDeltaH(max_dh)
  res_bfs, res_dij, res_ast, plot = None, None, None, False
  plot = True
  res_bfs = bfs_search()
  res_dij = dijkstra_search()
  res_ast = astar_search()
  showResults(res_bfs, res_dij, res_ast, plot)

cases = [
  ("T1-A", 3000, 7000, 3300,  7400),
  ("T1-B", 3500, 2000, 3250,  2350),
  ("T2-A", 2000, 5000, 3500,  7000),
  ("T2-B", 4000,10000, 5500, 14000),
  ("T3-A", 4500, 2400, 1000, 13500),
  ("T3-B", 3700,  500, 4500, 15500),
]

print("\n===", 'Main case', "===")
main()

for name, sx, sy, ex, ey in cases:
  print("\n===", name, "===")
  main(start_x=sx, start_y=sy, end_x=ex, end_y=ey, max_dh=0.25)
```
